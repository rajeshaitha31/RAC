...
public class Ecom_SignUpForDeals_DifferentVerifyEmail extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException, FilloException {
		...
		testdata = ExcelReader.GetData(sheetPath,
				"Select * from SignUpForDeals where TestCaseName='SignUpForDeals_DifferentVerifyEmail'");

	}

	@Test(groups = { "smoke", "regression" })
	public void SignUpForDeals_DifferentEmails() throws Throwable {
		ecom_homepage = new Ecom_HomePage(CommonVariables.CommonDriver.get());
		ecom_homepage.LaunchApplication();
		actionLib.WebdriverWaitForPage();

		while (testdata.next()) {
			ecom_homepage.signUpForDeals_DifferentEmail(testdata);
