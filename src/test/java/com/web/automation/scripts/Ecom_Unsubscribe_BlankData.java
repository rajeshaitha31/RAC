...
public class Ecom_Unsubscribe_BlankData extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException, FilloException {
		...
		testdata = ExcelReader.GetData(sheetPath,
				"Select * from SignUpForDeals where TestCaseName='Unsubscribe_Deals_BlankData'");

	}

	@Test(groups = { "smoke", "regression" })
	public void Unsubscribe_BlankData() throws Throwable {
		ecom_homepage = new Ecom_HomePage(CommonVariables.CommonDriver.get());
		ecom_homepage.LaunchApplication();
		actionLib.WebdriverWaitForPage();

		while (testdata.next()) {
			ecom_homepage.unsubscribeForDeals_BlankData(testdata);
