...
public class Ecom_AMS_AccountProfile_EditPersonalInfo_Email_DifferentEmail extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException, FilloException {
		...
		testdata = ExcelReader.GetData(sheetPath,
				"Select * from TestData where TestCaseName='Ecom_AMS_AccountProfile_EditPersonalInfo_Email_DifferentEmail'");
	}
	
	@Test(alwaysRun = true, groups = { "smoke", "regression" })
  public void Ecom_AMS_AccountProfile_EditPersonalInfo_Email_DifferentEmail() throws Throwable {
	  ...
	  ecom_homepage = new Ecom_HomePage(CommonVariables.CommonDriver.get());
		...
		Assert.assertEquals(CommonVariables.CommonDriver.get()
				.findElement(EcomAMS_OR.ACH_ERRORMSGVALIDATION).getText(),"Email address does not match.");
		extentLogs.pass("Verify Message Email address does not match.", "Email address does not match.");
