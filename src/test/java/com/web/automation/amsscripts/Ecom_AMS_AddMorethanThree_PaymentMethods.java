...
public class Ecom_AMS_AddMorethanThree_PaymentMethods extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException, FilloException {
		...
		testdata = ExcelReader.GetData(sheetPath,
				"Select * from TestData where TestCaseName='Ecom_AMS_AddMorethanThree_PaymentMethods'");
	}
	
	@Test(alwaysRun = true, groups = { "smoke", "regression" })
  public void Ecom_AMS_AddMorethanThree_PaymentMethods() throws Throwable {
	  ...
	  ecom_homepage = new Ecom_HomePage(CommonVariables.CommonDriver.get());
		...
		amspage.listOfPaymentMethods();
        actionLib.Click(CommonVariables.CommonDriver.get().findElement(EcomAMS_OR.ADD_ANOTHERPAYMENTMETHOD));
		if(actionLib.isElementPresent(EcomAMS_OR.MORETHAN3PAYMENTS_POPUP)) {
			String alert = CommonVariables.CommonDriver.get().findElement(EcomAMS_OR.MORETHAN3PAYMENTS_POPUP).getText();
			extentLogs.info("Verify ErrorMessage", alert);
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(EcomAMS_OR.MORETHAN3PAYMENTS_CLOSEPOPUP));
		}
