...
public class Ecom_AMS_AddCreditCardPaymentMehod_ValidData extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException, FilloException {
		...
		testdata = ExcelReader.GetData(sheetPath,
				"Select * from TestData where TestCaseName='Ecom_AMS_AddPaymentMehod_CreditCard_validData'");
	}
	
	@Test(alwaysRun = true, groups = { "smoke", "regression" })
  public void Ecom_AMS_AddCreditCardPaymentMehod_ValidData() throws Throwable {
	  ...
	  ecom_homepage = new Ecom_HomePage(CommonVariables.CommonDriver.get());
		...
		actionLib.WebdriverWaitForPage();
		amspage.fillcreditcardpersonaldata(Cardholdername);
		if(CommonVariables.CommonDriver.get().findElement(EcomAMS_OR.PAYMENTMETHODADDED_SUCCESSMSG).getText().contains("Your payment method has been added."));
		 extentLogs.pass("AddCreditCard", "CreditCardPaymentMethod added successfully for user  : " + Cardholdername );
