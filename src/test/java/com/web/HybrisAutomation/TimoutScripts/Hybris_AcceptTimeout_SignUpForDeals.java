...
public class Hybris_AcceptTimeout_SignUpForDeals extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException,FilloException {
		//Login Details
		      ...
		      this.Email = ExcelReader.fn_GetCellData(sheetPath, sheetName, 2,"Email"); 
			  this.VerifyEmail = ExcelReader.fn_GetCellData(sheetPath, sheetName,2, "VerifyEmail");
			  this.FirstName = ExcelReader.fn_GetCellData(sheetPath, sheetName, 2, "FirstName");
			  this.LastName = ExcelReader.fn_GetCellData(sheetPath, sheetName, 2, "LastName");
			  this.ZipCode = ExcelReader.fn_GetCellData(sheetPath, sheetName, 2, "ZipCode");
	} 
	
	@Test(groups = { "smoke", "regression" })
	public void Hybris_AcceptTimeout_SignUpForDeals_Test() throws Throwable{
		...
		actionLib.Click(CommonVariables.CommonDriver.get().findElement(Timeouts_OR.TimeoutContinue_SignUpForDeals));
		flag=hybris_signupdealspage.FillData_SignUpForDeals(Email, VerifyEmail, FirstName, LastName, ZipCode);
		Assert.assertEquals(true, flag, "FillDataintheForm");
		flag=hybris_signupdealspage.VerifySignUpForDealsSucess();
	}
