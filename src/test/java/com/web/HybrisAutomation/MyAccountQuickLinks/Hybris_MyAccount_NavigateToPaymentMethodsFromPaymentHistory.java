...
public class Hybris_MyAccount_NavigateToPaymentMethodsFromPaymentHistory extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException,FilloException {
		//Login Details
			  ...
			  this.password = ExcelReader.fn_GetCellData(sheetPath, sheetName,6, "password");
...
public void Hybris_MyAccount_NavigateToPaymentMethodsFromPaymentHistory_Test() throws Throwable{
...
actionLib.WebdriverWaitForPage();
  ...
  flag=hybris_myaccount.NavigateToPaymentHistory(); 
  Assert.assertEquals(true, flag,"Navigated to Payment History"); 
  actionLib.WebdriverWaitForPage();
  actionLib.Click(CommonVariables.CommonDriver.get().findElement(MyAccount_OR.PAYMENTHISTORY_PAYMENTMETHODSQUICKLINK));
  
  if(CommonVariables.CommonDriver.get().findElement(MyAccount_OR.PAYMENTMETHODS_LABEL).isDisplayed()){
	  extentLogs.pass("PaymentMethods", "PaymentMethodsPageDisplayed");
  }
  else {
	  extentLogs.fail("PaymentMethods", "PaymentMethodsPageIsNotDisplayed");
  }
