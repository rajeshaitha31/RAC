...
public class Hybris_MyAccount_AddPaypalPaymentMehod_ValidData extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException, FilloException {
		...
		this.username = ExcelReader.fn_GetCellData(sheetPath, sheetName, 10, "username");
		this.password = ExcelReader.fn_GetCellData(sheetPath, sheetName, 10, "password");
		this.paymentType = ExcelReader.fn_GetCellData(sheetPath, sheetName, 10, "paymentType");
		this.PaypalEmailid = ExcelReader.fn_GetCellData(this.sheetPath, this.sheetName, 10, "PaypalEmailid");
		this.PaypalPassword = ExcelReader.fn_GetCellData(this.sheetPath, this.sheetName, 10, "PaypalPassword");
	...
	public void Hybris_MyAccount_AddBankPaymentMehod_ValidData_Test() throws Throwable {
		...
		actionLib.WebdriverWaitForPage();
		flag = hybris_myaccount.PaypalPayment(PaypalEmailid, PaypalPassword);
		Assert.assertEquals(true, flag, "Added Paypal Payment Method");
		actionLib.WebdriverWaitForPage();
