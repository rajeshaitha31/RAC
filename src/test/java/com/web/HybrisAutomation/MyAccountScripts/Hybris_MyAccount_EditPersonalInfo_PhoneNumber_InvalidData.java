...
public class Hybris_MyAccount_EditPersonalInfo_PhoneNumber_InvalidData extends TestEngineWeb {
	...
	public void beforeMethod() throws IOException,FilloException {
		//Login Details
			  ...
			  this.phoneNumber = ExcelReader.fn_GetCellData(sheetPath, sheetName,5, "contactNumber");
...
public void Hybris_MyAccount_EditPersonalInfo_PhoneNumber_InvalidData_Test() throws Throwable{
...
actionLib.WebdriverWaitForPage();
  ...
  Assert.assertEquals(true, flag,"User Signed In");
	 ...
	 CommonVariables.CommonDriver.get().findElement(MyAccount_OR.PERSONALINFO_PHONENUMBER).clear();
	 actionLib.type(CommonVariables.CommonDriver.get().findElement(MyAccount_OR.PERSONALINFO_PHONENUMBER), phoneNumber);
	 extentLogs.pass("EnterPhoneNumber","EnteredPhoneNumber");
	 ...
	 Assert.assertEquals(CommonVariables.CommonDriver.get()
				.findElement(MyAccount_OR.PERSONALINFO_CONTACTINFOERRORMSG).getText(),"Please enter a valid phone number.");
	  extentLogs.pass("VerifyErrorMessage", "Error Message 'Please enter a valid phone number.' matched with actual Message: Please enter a valid phone number.");
